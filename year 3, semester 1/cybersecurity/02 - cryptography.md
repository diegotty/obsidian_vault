...
### certificates
>[!info] process
![[Pasted image 20251013115220.png]]
the user’s certificate is hashed and that hash is encrypted with the *CA*’s (certificate authority) private key, to form a signature.
>- this is what happens in the browser ! the browsers know the public keys of the entities communicate with, and the use certificates to securely connect to a website and authenticate a user to a website. after the certificate sent by the site is verified (for use-case one), the browser creates a secure, encrypted connection. this is called *SSL/TLS encryption*
### digital envelope
it is a secure method for transmitting encrypted messages using both symmetric and asymmetric encryption !
>[!info] process
![[Pasted image 20251013114729.png|500]]
the receiver’s public key is used to encrypt the random symmetric key that encrypted the message
## random numbers
