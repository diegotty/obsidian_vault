...
creiamo un modello più semplice (slide 14) dato che le operazioni tra lock e unlock non ci interessano


in corrispondenza di una unlock(scrittura), per ogni transazione, mettiamo una funzione fittizia, che prende come parametro tutti gli item che la transazione ha letto fino a quel momento, con il valore dell’item quando è stato letto (non quando è stato scritto !)